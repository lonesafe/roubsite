...
public class Engine {
	...
	public Engine() {
		...
		debug = ConfUtils.getBooleanConf("RoubSite.smarty.debug");
		if ("off".equals(ConfUtils.getStringConf("RoubSite.smarty.cache", "off"))) {
			cache = false;
		}
		setEncoding(ConfUtils.getStringConf("RoubSite.smarty.encoding", "utf-8"));
		setLeftDelimiter(ConfUtils.getStringConf("RoubSite.smarty.left.delimiter", "{"));
		setRightDelimiter(ConfUtils.getStringConf("RoubSite.smarty.right.delimiter", "}"));

		StringBuilder sb = new StringBuilder(64);
		sb.append(Parameter.class.getPackage().getName());
		int len = sb.length();
		sb.append(".function");
		String value = ConfUtils.getStringConf("RoubSite.smarty.package.function",
				"com.roubsite.smarty4j.statement.function");
		if (value != null) {
		...
		sb.setLength(len);
		sb.append(".modifier");
		value = ConfUtils.getStringConf("RoubSite.smarty.package.modifier", "com.roubsite.smarty4j.statement.modifier");
		if (value != null) {
			sb.append(';').append(value);
