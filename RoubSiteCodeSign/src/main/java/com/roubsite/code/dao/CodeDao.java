...
public class CodeDao extends EntityDao implements ICodeDao {
	...
	public void createCodeFile(String tableName, String keyField, LinkedList<BeanField> searchFields,
		...
		String pack = ConfUtils.getConf("global.group." + groupName, "config.properties",
				new String[] { "RoubSite", "global", "group", groupName });
		String packPath = pack.replaceAll("\\.", "/");
		String basePath = ConfUtils.getConf("codeSign.outputPath", "codeSign.properties",
				new String[] { "RoubSite", "codeSign", "outputPath" });
		...
		cs.replaceAll(daoTpl,
				basePath + "/src/main/java/" + packPath + "/dao/" + _modeName + "/" + modeName + "Dao.java");
		// 生成dao结束

		// 生成dao接口
		String iDaoTpl = getTplString("codeTemplate/dao/ICodeDao.tpl");
		cs.replaceAll(iDaoTpl,
				basePath + "/src/main/java/" + packPath + "/dao/" + _modeName + "/" + "I" + modeName + "Dao.java");
		// 生成dao接口结束
		...
		cs.replaceAll(insertJsp, basePath + "/src/main/webapp/templates/" + groupName + "/" + _modeName + "/insert"
				+ modeName + ".html");
		cs.replaceAll(updateJsp, basePath + "/src/main/webapp/templates/" + groupName + "/" + _modeName + "/update"
				+ modeName + ".html");
